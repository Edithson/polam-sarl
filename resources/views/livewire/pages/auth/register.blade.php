...
$register = function () {
    ...
    $validated['permissions'] = [
        'articles' => AccessLevel::AUTHOR->value,   // Peut rédiger ses articles
        'contacts' => AccessLevel::NONE->value,     // Ne voit pas les contacts
        'settings' => AccessLevel::NONE->value,     // Ne voit pas les paramètres
        'profile'  => AccessLevel::AUTHOR->value,   // Peut gérer son propre compte
    ];
...
<div class="min-h-screen flex items-center justify-center bg-[#F5F6FA] py-12 px-4 sm:px-6 lg:px-8 font-['DM_Sans',sans-serif]">
    <div class="max-w-md w-full bg-white rounded-sm shadow-sm border border-[#E8EAF0] p-8">
        ...
        <div class="flex flex-col items-center justify-center text-center mb-8">
            ...
            <h2 class="text-2xl font-bold font-['Syne',sans-serif] text-gray-900">Bienvenue parmi nous !</h2>
            <p class="mt-2 text-sm text-gray-500">Veuillez entrer vos informations pour créer votre compte.</p>
        </div>

        <form wire:submit="register" class="space-y-4">

            <div>
                <label for="name" class="block text-[10px] font-bold font-['Syne',sans-serif] tracking-wider uppercase text-gray-500 mb-1.5">Nom complet</label>
                <div class="relative group">
                    ...
                    <input wire:model="name" id="name" type="text" name="name" required autofocus autocomplete="name" class="w-full pl-10 pr-4 py-2.5 bg-[#F5F6FA] border border-[#E8EAF0] rounded-sm font-['DM_Sans',sans-serif] text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-all" placeholder="Jean-Marc Essono">
                </div>
            ...
            <div>
                ...
                <div class="relative group">
                    ...
                    <input wire:model="email" type="email" name="email" id="email" required autocomplete="username" class="w-full pl-10 pr-4 py-2.5 bg-[#F5F6FA] border border-[#E8EAF0] rounded-sm font-['DM_Sans',sans-serif] text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-all" placeholder="nom@entreprise.cm">
                </div>
            ...
            <button type="submit" class="w-full flex justify-center items-center gap-2 mt-6 py-3 px-4 border border-transparent rounded-sm shadow-sm text-xs font-bold font-['Syne',sans-serif] uppercase tracking-wider text-white bg-orange-500 hover:bg-[#EA580C] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 transition-all active:scale-[0.98]">
                S'inscrire
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M14 5l7 7m0 0l-7 7m7-7H3" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
            </button>
        </form>

        <p class="mt-6 text-center text-xs text-gray-500">
            Déjà un compte ? <a href="{{ route('login') }}" class="text-orange-500 font-bold hover:text-[#EA580C] transition-colors" wire:navigate>Se connecter</a>
        </p>
    </div>
</div>
